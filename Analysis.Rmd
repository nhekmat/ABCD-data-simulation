...
### 1. Model 1 observing effects of covariates without heat as a moderator
...
F_to_eta2(32.6189 ,1,4122.7) #ab_g_stc__cohort_sex
F_to_eta2(20.8333 ,1, 3964.1) #mh_y_upps__pers_sum
F_to_eta2(86.8056,1,3907.4 ) #mh_y_upps__nurg_sum
F_to_eta2(7.4252,1,324.7) # le_l_coi__addr1__coi__total__national_score  
...
# 2. Moderating effect of heat for model 1 only for sex
...
F_to_eta2(20.5901 ,1,3962.2 ) #mh_y_upps__pers_sum                          
F_to_eta2(88.0050 ,1,3906.1 ) #mh_y_upps__nurg_sum                         
F_to_eta2(7.5574,1,309.8  ) #le_l_coi__addr1__coi__total__national_score  
F_to_eta2(9.8379  ,1,4132.1  ) #days_g_90_site:ab_g_stc__cohort_sex   
# Interpretation of values
# The a linear mixed effects model analyzing the relationship between externalizing symptoms and included the moderating effect of extreme heat on cohort sex. The model included area deprivation national score, behavioral inhibition score, days exposed to extreme heat, UPPS perserverance score, UPPS negative urgency score, and child oppertunity index national score.
# Analysis revealed a significant moderating effect of extreme heat on cohort sex (beta =  -0.24, SE = 0.005, t = -2.75, p = 0.002). 
# The main effects of the model revealed that UPPS subscales of Negative Urgency (beta = 0.31, SE = 0.03, t = 9.38, p < 0.001) and Perseverance (beta = 0.17, SE = 0.04, t = 4.54, p < 0.001), child opportunity index (beta = -0.01, SE = 0.005, t = - 2.75, p = 0.006) was positively associated with externalizing behaviors. 


# Unsure of whether it's relevant to report on multicolinearity here

```


### 3. Model 1 observing effects of covariates with heat as a moderator
...
# Analysis revealed a significant moderating effect of extreme heat on cohort sex and externalizing behaviors (beta =  -0.25, SE = 0.08, t = -3.19, p = 0.001).

#Checking to see if relationship gets stronger the more days exposed to extreme heat for sex
slopes1a = emtrends(
  model_1a,
  specs = ~ab_g_stc__cohort_sex,
  var = "days_g_90_site",
  # at = list(ab_g_stc__cohort_sex = c(1,2,3,4,5)),
  pbkrtest.limit = 4148,
  adjust = "fdr")
print(slopes1a)

effectssize1a = standardize_parameters(model_1a,
                                       method = "refit")
print(effectssize1a)

F_to_eta2(6.0395,1,4007.4) #for perseverence
F_to_eta2(29.0626 ,1,3932.9) #for negative urgency
F_to_eta2(10.1993, 1, 4127.7) # days_g_90_site:ab_g_stc__cohort_sex
# poweranalysis1 <- powerSim(model_1a,
#                            test = fixed ("days_g_90_site:ab_g_stc__cohort_sex2", method = "kr"),
#                            nsim = 1000)
# print(poweranalysis1)

```

### 4. Model 1 observing effects of covariates with heat as a moderator for males
...
# Further analysis looked at the male sex cohort separately to understand the moderating effect of extreme heat on externalizing behaviors. The model revealed that extreme heat was significantly associated with externalizing behaviors in the male cohort (beta = 0.16, SE = 0.07, t = 2.40. p = 0.02).

#Effects size analysis
standardize_parameters(model_1a_male, method = "refit")
F_to_eta2(16.268,1,2047.4) #mh_y_upps__nurg_sum


# poweranalysis1a_nurg <- powerSim(model_1a_male,
#                            test = fixed ("mh_y_upps__pers_sum", method = "kr"),
#                            nsim = 1000)
# 
# 
# print(poweranalysis1a_nurg)
...
### 5. Model 1 observing effects of covariates with heat as a moderator for females
...
# Looking at the female sex, the model revealed the moderating effect of extreme heat on externalizing behaviors for the female sex cohort was not significant (beta = - 0.09, SE = 0.06, t = -1.55, p = 0.12).

standardize_parameters(model_1a_female, method = "refit")
F_to_eta2(6.1878 ,1,1932.38) #mh_y_upps__pers_sum
F_to_eta2(13.6373,1,1890.35) #mh_y_upps__nurg_sum
F_to_eta2(6.2569,1,915.30) #le_l_coi__addr1__coi__total__national_score
F_to_eta2(4.1919 ,1,1830.61) #days_g_90_site:le_l_coi__addr1__coi__total__national_score 
# poweranalysis1a_femaleupps <- powerSim(model_1a_female,
#                            test = fixed ("mh_y_upps__pers_sum", method = "kr"),
#                            nsim = 1000)
# poweranalysis1a_femalenurg <- powerSim(model_1a_female,
#                            test = fixed ("mh_y_upps__pers_sum", method = "kr"),
#                            nsim = 1000)
# poweranalysis1a_femalecoi <- powerSim(model_1a_female,
#                            test = fixed ("mh_y_upps__pers_sum", method = "kr"),
#                            nsim = 1000)

# print(poweranalysis1a_femaleupps)
# print(poweranalysis1a_femalenurg)
# print(poweranalysis1a_femalecoi)



```

 # 6. Below is just testing for model 2 which includes the moderating effect of heat on all covariates? Unsure how to interpret
